...
The server is open (no login) until the first user is created via `POST /api/users` (that user becomes `admin`). From then on every `/api` call except `/api/health`, `/api/login`, `/api/auth/state` and the public `/s/` share routes needs a session: `Authorization: Bearer <token>` from `POST /api/login`, or the `chitra_session` cookie the login response sets (browsers). Admins manage users (`/api/users`). Members may add and organise (upload, favourites, albums, people, shares) but never delete or rewrite files: `_member_blocked()` in the auth hook answers 403 to every DELETE, to trash/restore/purge, and to edit/rotate/move/organize/rescan; the Trash listing (`?trashed=1`) is admin-only. `/api/auth/state` reports `can_delete` and the clients hide those controls when it is false.

Admins (only) have a Locked folder: `POST /api/media/lock {ids}` / `POST /api/user_albums/<id>/lock` / `POST /api/albums/lock {album}` set `media.private_to` / `albums.private_to` to the caller; every lock/unlock/listing route calls `_require_admin()`, so members never see a Locked entry. Locked items are excluded from every listing (`visible_clause()`, plus a guard in the auth hook that 404s per-item routes) and from public shares; the owner sees them only in `GET /api/media?locked=1` (and, if trashed, in their own Trash) after `POST /api/locked/unlock {password}` opened the session; other admins never see them, in the Trash included. The unlock is a sliding window: each request touching locked content extends it by `UNLOCK_IDLE_SECONDS` (60), so a minute of inactivity relocks server-side; clients also run a one-minute idle timer that calls `POST /api/locked/lock` and leaves the view, and ask for the password on every open. Any new query that lists media must append `private_to IS NULL`.

Clients: the web app, Android and iOS are all wired (sign-in prompt on 401, Locked view with idle relock, lock/unlock actions, member gating via `can_delete`, account management in Settings). On iOS the token lives in `Data/Auth.swift` and the session state in `UI/AuthSession.swift`; the Swift is written on Linux without a compiler, so build it on the Mac (`xcodegen generate` + `xcodebuild`, or `ios/Tools/logic-check.sh` for the model/decoding assertions) before trusting a change.
