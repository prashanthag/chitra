...
The server is open (no login) until the first user is created via `POST /api/users` (that user becomes `admin`). From then on every `/api` call except `/api/health`, `/api/login`, `/api/auth/state` and the public `/s/` share routes needs a session: `Authorization: Bearer <token>` from `POST /api/login`, or the `chitra_session` cookie the login response sets (browsers). Admins manage users (`/api/users`). Members are viewers: the auth hook answers 403 to every non-GET request from a member except phone backup (`/api/upload`, `/api/upload/check`), their own password change and logout (`_MEMBER_WRITE_PATHS`); `/api/auth/state` reports `can_edit` and the clients hide editing controls when it is false.
